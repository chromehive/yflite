...
function makePage($name) {
    ...
    $dirPath = __DIR__ . '/../views/' . implode('/', $parts);
    
    // Create directory if needed
    if (!empty($parts) && !is_dir($dirPath)) {
        mkdir($dirPath, 0777, true);
        echo "📁 Directory created: " . str_replace(__DIR__ . '/../', '', $dirPath) . "\n";
    }
    
    $pagePath = ($dirPath ? $dirPath . '/' : __DIR__ . '/../views/') . $fileName . '.php';
    
    if (file_exists($pagePath)) {
        echo "⚠️  Page already exists: views/" . str_replace(__DIR__ . '/../views/', '', $pagePath) . "\n";
        return false;
    }
    ...
    echo "✅ Page created: views/" . str_replace(__DIR__ . '/../views/', '', $pagePath) . "\n";
    return true;
}
...
function makeCrud($name, $fields = null) {
    ...
    $modelDir = __DIR__ . '/../models';
    $viewDir = __DIR__ . '/../views/' . $resource;
